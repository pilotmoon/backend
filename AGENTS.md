...
sending the raw file body to `POST /files?name=<path>`; retrieve metadata via
`GET /files/{fileId}` and stream binary data through
`GET /files/download/{file/path.ext}` with `application/octet-stream`. Remove a
file entirely with `DELETE /files/{fileId}`—names become available for reuse
after deletion. Keep shared schemas in `src/common/fileSchemas.ts`, controllers
in `src/rolo/controllers/filesController.ts`, and update tests whenever storage
